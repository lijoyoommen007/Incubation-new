  ...
  return (
    <>
      {val.map((slotObj, index) => {
        if (slotObj.isBooked || slotArr.includes(slotObj.slot)) return <div key={slotObj.slot} className='w-16 h-16 bg-red-700'>  </div>;
        else return <div key={slotObj.slot} onClick={(e) => {
          setModal(true)
          setIndex({ val: val, index: index })
        }} className='w-16 h-16 bg-blue-500 hover:bg-yellow-600'></div>;
      })}
    </>
  );
}
